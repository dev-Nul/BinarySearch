...
void sortArray(int unorderedArray[], int len, bool reverse = false)
{
    ...
    while (!sorted)
    {
        ...
        for (int i = 0; i < len; ++i)
        {
            ...
            if (i != len - 1)
            {
                // If there is a number after this, is it smaller than the current one?
                if (unorderedArray[i + 1] < unorderedArray[i])
                {
                    // If the number following this one is smaller, swap them.
...
string formatArray(int arr[], int arrLength)
{
    ...
    // Also add a period to the end of the string.
    arrayAsString += ".\n";
